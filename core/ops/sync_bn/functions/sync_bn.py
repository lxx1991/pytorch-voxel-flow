...
class Synchronize:
    ...
    def init(device_num):
        ...
        Synchronize.result_ready = [
            threading.Event() for _ in range(device_num)
        ]
        ...
        def _worker():
            while (True):
                ...
                cuda.nccl.all_reduce(Synchronize.data_list,
                                     Synchronize.result_list)
                cuda.synchronize()
    ...
    def all_reduce_thread(input):
        ...
        with torch.cuda.device(input_device):
            Synchronize.result_list[input_device] = input.new().resize_as_(
                input).zero_()
...
class _sync_batch_norm(Function):
    ...
    def backward(self, grad_output):
        ...
        sync_bn_lib.bn_backward_after_allreduce(
            ...
            weight, self.x_std, grad_input, allreduce_num)
...
def sync_batch_norm(input,
    ...
    """
    return _sync_batch_norm(momentum, eps)(input, running_mean, running_var,
                                           weight, bias)
